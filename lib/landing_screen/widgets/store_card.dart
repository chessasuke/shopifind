...
import 'package:shopifind/store_editor/controller/store_controller.dart';
import 'package:shopifind/store_editor/model/store.dart';
import 'package:shopifind/store_editor/store_editor_screen.dart';
...
class StoreCard extends ConsumerWidget {
  ...
  void _onTap(WidgetRef ref, BuildContext context) async {
    if (store == null) {
      ...
      // ref.read(currentStoreIdProvider.notifier).state = newStore.id!;
      // print('======== current store id: ${ref.read(currentStoreIdProvider)}');
      Navigator.push<void>(
        context,
